...
const int L_S =2;  // cb dò line trái
const int S_S1 =13;  // cb dò line giữa
const int S_S2 =9;  // cb dò line giữa
const int R_S =10; //cb dò line phải

int left_sensor_state;// biến lưu cảm biến hồng ngoại line trái
int s1_sensor_state;   // biến lưu cảm biến hồng ngoại line giữa1
int s2_sensor_state;   // biến lưu cảm biến hồng ngoại line giữa2
...
void setup() {
  ...
  Serial.begin(9600); 
  analogWrite(motorAspeed, 20); // tốc độ động cơ a ban đầu 120 ( 0 - 255)
  analogWrite(motorBspeed, 20);// tốc độ động cơ b ban đầu 120 ( 0 - 255)
  delay(3000);                              
...
void loop() {
  ...
  Serial.println(distance);
   ...
   s1_sensor_state = digitalRead(S_S1);
   s2_sensor_state = digitalRead(S_S2);
   right_sensor_state = digitalRead(R_S);

if ((digitalRead(L_S) == 0)&&(digitalRead(S_S1) == 1)&&(digitalRead(S_S2) == 1)&&(digitalRead(R_S) == 0)){forward();}// đi tiến
if ((digitalRead(L_S) == 0)&&(digitalRead(S_S1) == 0)&&(digitalRead(S_S2) == 1)&&(digitalRead(R_S) == 0)){forward();}// đi tiến
if ((digitalRead(L_S) == 0)&&(digitalRead(S_S1) == 1)&&(digitalRead(S_S2) == 0)&&(digitalRead(R_S) == 0)){forward();}// đi tiến


if ((digitalRead(L_S) == 1)&&(digitalRead(S_S1) == 1)&&(digitalRead(S_S2) == 0)&&(digitalRead(R_S) == 0)){turnLeft();} // rẻ trái
if ((digitalRead(L_S) == 1)&&(digitalRead(S_S1) == 1)&&(digitalRead(S_S2) == 1)&&(digitalRead(R_S) == 0)){turnLeft();} // rẻ trái
if ((digitalRead(L_S) == 1)&&(digitalRead(S_S1) == 0)&&(digitalRead(S_S2) == 0)&&(digitalRead(R_S) == 0)){turnLeft();} // rẻ trái

if ((digitalRead(L_S) == 0)&&(digitalRead(S_S1) == 0)&&(digitalRead(S_S2) == 0)&&(digitalRead(R_S) == 1)){turnRight();} // rẻ trái
if ((digitalRead(L_S) == 0)&&(digitalRead(S_S1) == 1)&&(digitalRead(S_S2) == 1)&&(digitalRead(R_S) == 1)){turnRight();} // rẻ trái
if ((digitalRead(L_S) == 0)&&(digitalRead(S_S1) == 0)&&(digitalRead(S_S2) == 0)&&(digitalRead(R_S) == 1)){turnRight();} // rẻ trái


if ((digitalRead(L_S) == 1)&&(digitalRead(S_S1) == 1)&&(digitalRead(S_S2) == 1)&&(digitalRead(R_S) == 1)){Stop();} // stop
 ...
 if(distance < stop_distance) // nếu khoảng cách nhỏ hơn giới hạn 
 {
  ...
  analogWrite (motorAspeed,15);
  analogWrite (motorBspeed,15);
  delay(400);
  ...
  digitalWrite (motorB1,LOW);
  digitalWrite(motorB2,LOW);
  analogWrite (motorAspeed, 20);
  analogWrite (motorBspeed, 20);
  delay(1000);
